...
export const generateMealPlan: RequestHandler<unknown, unknown, MealPlanBody, unknown> = async (req, res, next) => {
    ...
    try {
        ...
        const pipeline = [
            { $match: { $in: [new RegExp('^' + preference, 'i')] } },
            { $sample: { size: mealsPerDay } }
        ]
        // filtering
        const meals = MealKitModel.aggregate(pipeline);
        const currentDate = new Date();
        let mealKits: Kit[] = [];
        if (mealsPerDay === 1) {
            const mealTimes = ['lunch', 'supper'];
            const selectedMealKits = (await meals).filter(meal => meal.weight === 'heavy') as unknown as MealKitDB[];
            ...
            const heavyMealKits = (await meals).filter(meal => meal.weight === 'heavy') as unknown as MealKitDB[];
            const lightMealKits = (await meals).filter(meal => meal.weight === 'light') as unknown as MealKitDB[];
